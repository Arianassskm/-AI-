...
export function MedicationInteractionModal({
  isOpen,
  onClose,
}: MedicationInteractionModalProps) {
  ...
  const handleAnalyze = async () => {
    ...
    setIsLoading(true);
    const result = await analyzeMedicationInteraction(
      medicationOne,
      medicationTwo
    );
    setIsLoading(false);
      ...
      <div className="p-4 space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            placeholder={
              interactionType === "drug" ? "请输入药品名称" : "请输入食物名称"
            }
          />
        </div>
