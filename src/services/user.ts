...
export const userService = {
   ...
   * 更新用户基本信息
   */
  async updateUserProfile(userId: string, profile: Partial<User>): Promise<UserResponse<User>> {
    try {
      const { data, error } = await supabase
        .from('users')
        .update(profile)
      ...
      return {
        data: null,
        error: { message: err instanceof Error ? err.message : '更新用户信息失败' }
      };
    }
  },
  ...
  async getHealthRecord(userId: string): Promise<UserResponse<UserHealth>> {
    try {
      ...
      return {
        data: null,
        error: { message: err instanceof Error ? err.message : '获取健康记录失败' }
      };
    }
  },
  ...
  async updateHealthRecord(
    ...
    try {
      ...
      return {
        data: null,
        error: { message: err instanceof Error ? err.message : '更新健康记录失败' }
      };
    }
  },
   ...
   * 获取用户健康趋势数据
   */
  async getHealthTrends(userId: string): Promise<UserResponse<{
    yearlyUsage: number;
    averageSteps: number;
    riskScore: number;
  }>> {
