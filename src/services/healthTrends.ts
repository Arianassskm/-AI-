...
export const healthTrendsService = {
  ...
  async getHealthTrends(userId: string): Promise<HealthTrendsResponse> {
    try {
      ...
      // 计算平均步数
      const averageSteps = stepData.length > 0
        ? Math.round(stepData.reduce((sum, record) => sum + record.steps, 0) / stepData.length)
        : 0;
      ...
      planData.forEach(plan => {
        ...
        if (start <= yearEnd && end >= yearStart) {
          ...
          medicationDays += (effectiveEnd.getTime() - effectiveStart.getTime()) / (1000 * 60 * 60 * 24);
        }
      });
      ...
      return {
        data: null,
        error: { message: err instanceof Error ? err.message : '获取健康趋势数据失败' }
      };
    }
  },
    ...
    try {
      const { error } = await supabase
        ...
        .insert([{
          ...
          blood_pressure_systolic: trend.bloodPressureSystolic,
          blood_pressure_diastolic: trend.bloodPressureDiastolic,
          blood_sugar: trend.bloodSugar,
          heart_rate: trend.heartRate
        }]);
      ...
      return {
        error: { message: err instanceof Error ? err.message : '添加健康趋势记录失败' }
      };
    }
  }
};
 ...
 */
async function calculateRiskScore(userId: string, trends: HealthTrend[]): Promise<number> {
  try {
    // 获取用户健康记录
    ...
    if (trends.length > 0) {
      ...
      // 血压评估
      if (latestTrend.bloodPressureSystolic > 140 || latestTrend.bloodPressureDiastolic > 90) {
        score += 10;
      }
